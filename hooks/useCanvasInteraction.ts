...
export function useCanvasInteraction(
      ...
      if (el.type === "image") {
        if (x >= el.x && x <= el.x + el.width && y >= el.y && y <= el.y + el.height) {
          if (x >= el.x + el.width - 20 && y >= el.y + el.height - 20) {
            setInteraction({ ...interaction, resizingElementId: el.id, resizeStart: { w: el.width, h: el.height, x, y } });
            e.currentTarget.setPointerCapture(e.pointerId);
            canvas.style.cursor = "nwse-resize";
            break;
          } else {
            setInteraction({ ...interaction, draggingElementId: el.id, dragOffset: { x: x - el.x, y: y - el.y } });
            e.currentTarget.setPointerCapture(e.pointerId);
  ...
  // Keep track of the requestAnimationFrame ID and latest pointer coordinates/state via refs to avoid closure lag or stale state in deferred RAF execution.
  ...
  return { interaction, setInteraction, handlePointerDown, handlePointerMove, handlePointerUp };
}
