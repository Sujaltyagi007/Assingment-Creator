...
export function useEditorLogic() {
  ...
  useEffect(() => { prevPagesLength.current = doc.pages.length }, [doc.pages.length]);
  ...
  const { zoom, handleZoomIn, handleZoomOut, handleResetZoom, handleFitWidth, handleTouchStart, handleTouchMove, handleTouchEnd } = useZoom(canvasRef);
  const { interaction, handlePointerDown, handlePointerMove, handlePointerUp, deleteSelectedImage } = useCanvasInteraction(canvasRef, activePage, dispatch);
  ...
  useEffect(() => {
    ...
    const renderId = requestAnimationFrame(() => {
      ...
      if (maxRequiredPages > doc.pages.length) { dispatch({ type: "ENSURE_PAGE_COUNT", count: maxRequiredPages }); }
      if (targetPageIndex !== undefined && targetPageIndex !== activePageIndex) { setUiState(s => ({ ...s, pendingActivePageIndex: targetPageIndex })); }
      if (uiState.lastEditIndex !== null) { setUiState(s => ({ ...s, lastEditIndex: null })); }
    });
  ...
  const handleAddImage = (src: string, width: number, height: number) => {
    dispatch({ type: "ADD_IMAGE", pageId: activePage.id, src, width, height, });
  };
  ...
  return {
    ...
    isLoaded, zoomMode: zoom.mode, zoomLevel: zoom.level,
    selectedImageId: interaction.selectedImageId,
